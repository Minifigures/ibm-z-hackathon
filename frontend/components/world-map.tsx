  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      if (existing) {
        ...
        // Click anywhere on the map: if it hits a country dot, lock that selection.
        // Clicking empty ocean clears the lock (hover-preview resumes).
        map.on("click", (ev) => {
          const feats = map.queryRenderedFeatures(ev.point, { layers: ["regions-hit"] });
          if (feats.length) {
            const iso3 = feats[0].properties?.iso3 as string;
            lockedRef.current = iso3;
            onSelectRef.current(iso3);
          } else {
        ...
        map.on("mousemove", (ev) => {
          ...
          // If the user has locked a selection by clicking, hover does not override it.
